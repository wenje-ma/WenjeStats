sp <- function(n, p, ini=NA,
  ...
  if (all(is.na(dist.samp))){
    ...
    if (all(is.na(ini))){
      if (p==1){
        ini <- matrix(randtoolbox::sobol(n,p,scrambling=F),ncol=1)
      }else{
        ini <- randtoolbox::sobol(n,p,scrambling=F)
      }
    ...
    if (all(is.na(ini))){
      ...
      ini <- stats::kmeans(dist.samp[sample(1:nrow(dist.samp),nn,F),],centers=n,iter.max=100)$centers
      ini.flg <- FALSE
    }
    if (all(is.na(bd))){
      bd <- matrix(NA,nrow=p,ncol=2,byrow=T)
